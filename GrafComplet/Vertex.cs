...
namespace GrafComplet
{
    internal class Vertex
    {
        ...
        {
            handler.FillEllipse(new SolidBrush(fillColor), 
                X - size, Y - size, 2 * size + 1, 2 * size + 1);//umplem elipsa de culoare
            handler.DrawEllipse(Pens.Black, X - size, 
                Y - size, 2 * size + 1, 2 * size + 1);//o desenam
            handler.DrawString(name, new Font("Arial", 
                15, FontStyle.Bold), new SolidBrush(Color.White), 
                new Point(X - size / 2, Y - size / 2));//desenam numele
